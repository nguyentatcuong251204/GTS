...
#define N 100
#define input "input_nghich_dao_gj.txt"
...
void output_screen(MT A) {      // in ra ma tráº­n
    cout << "******************************************************************************\n";
    for (int i = 0; i < A.row; i++) {
        for (int j = 0; j < A.col; j++) {
...
MT create_unit_matrix(MT &A) {      // táº¡o ma tran don vi
    for (int i = 0; i < A.row; i++) {
        for (int j = 0; j < A.col; j++) {
            if (i == j) A.a[i][j] = 1;
            else A.a[i][j] = 0;
...
void inverse_matrix_GJ(MT A) {      // tim nghich dao cua ma tran bang gauss - jordan
    ...
    for (m = 0; m < A.row; m++) {       
        ...
        if (index_element_solution(A, index, row_solution, col_solution)) {     // neu tim duoc phan tu giai
            cout << "\nChon phan tu giai la: " << A.a[row_solution][col_solution] << " o vi tri ("<<row_solution<<","<<col_solution<<")"<<endl;
            for (int i = 0; i < A.row; i++) {       
                if (i != row_solution) {
    ...
    if (m < A.row)
        ...
        for (int i = 0; i < A.row; i++) {       // doi vi tri cac hang sao cho a_ii != 0
            if (fabs(A.a[i][i]) < 1e-4) {       // neu a[i][i] == 0, dÃ¹ng < 1e-4 la vi a[i][i] co sai so
                for (int h = i + 1; h < A.row; h++) {
                    if (fabs(A.a[h][i]) > 1e-4) {       // neu a[i][i] != 0, dÃ¹ng > 1e-4 la vi a[i][i] co sai so
                        swap_row(A, h, i);  
